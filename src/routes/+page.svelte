<script>
	...
	export async function clickEndGame() {
		...
		// here we need to do a query to the backend to delete the file
		const resp = await fetch(PUBLIC_BACKEND_BASE_URL + `/delete-file/${userId}`, {
			method: 'DELETE',
			headers: {
				'Content-Type': 'application/json'
			}
	...
	export async function getReply() {
		...
		const resp = await fetch(PUBLIC_BACKEND_BASE_URL + `/get-reply/${userId}`, {
			// note to delete userid if it doesnt work
			method: 'POST',
			mode: 'cors',
			headers: {
				'Content-Type': 'application/json'
...
<div class="bg-black min-h-screen flex flex-col items-center justify-center">
	...
	<!-- MODAL 2 -->
	<div class=" w-screen h-screen flex-col game-space bg-white">
		<div
